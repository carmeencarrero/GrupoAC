	...
	float velocidadGiro = 10 / (ldata[l*4].dist / 5000);
       if (velocidadGiro > 2) //para asegurarnos que el robot va a 2 como velocidad maxima
            velocidadGiro = 2;
                    ...
                    }else if (angulo<0.5){ //neg
                            differentialrobot_proxy->setSpeedBase(0,-velocidadGiro);
}
			else if (angulo>0.5){ //pos
			differentialrobot_proxy->setSpeedBase(0,velocidadGiro);
                    }
			
		    //ha llegado al punto, por lo que lo quita y coge el siguiente
                    if(dist < 100)
                    {
                        path.pop_front();
                        
                        if(path.size() != 0) //si es el ultimo punto, no coge el siguiente porque no hay
                        currentPoint = path.front(); 
			//paramos el robot
